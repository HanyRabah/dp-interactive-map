...
export default function DrawMap() {
	...
	const handleDrawCreate = useCallback(
		({ features }: { features: Feature[] }) => {
			...
			const newPolygon: Polygon = {
				...
				style:
					feature.geometry.type === ("Polygon" as unknown as GeoJsonPolygon)
						? {
								fillColor: "#000000",
	...
	const handleUpdateDrawing = useCallback((e: { features: Feature[] }) => {
				...
				{isSearching && searchResults.length > 0 && (
					<div className="mt-2 bg-white rounded-lg shadow-lg max-h-[300px] overflow-y-auto">
						<div className="p-2 flex justify-between items-center border-b">
							...
							<button onClick={() => setIsSearching(false)} className="text-gray-500 hover:text-gray-700">
								<X size={16} />
							</button>
						</div>
			...
			<Box sx={{ width: "66.666667%", position: "relative" }}>
				...
				{(selectedFeature || selectedMarker) && (
					<Box sx={{ position: "absolute", bottom: 16, right: 16, zIndex: 10 }}>
						<DeleteButton
							...
							type={mode === MODES.VIEW ? "view" : mode === MODES.MARKER ? "marker" : "feature"}
						/>
					</Box>
				)}
				...
				{editingPolygonId && (
					<Box sx={{ position: "absolute", top: 16, left: 16, zIndex: 10 }}>
						<Button variant="contained" color="primary" onClick={handleFinishEditing}>
							Finish Editing
						</Button>
					</Box>
				)}
