...
export const GlobePOIMarkers: React.FC<GlobePOIMarkersProps> = ({ pois, onPOIClick, showMarkers, project }) => {
	...
	const fetchRouteDetails = async (poi: POI) => {
		...
		try {
			...
			setRouteDetails(prev => ({
				...
				[poi.id]: {
					driving:
						drivingRoute && !("message" in drivingRoute)
							? {
							...
							: null,
					walking:
						walkingRoute && !("message" in walkingRoute)
							? {
				...
				return (
					<Marker
						...
						<div
							...
							{isHovered && (
								<div
									className="absolute opacity-100 transition-all duration-200 
                              bottom-full left-1/2 -translate-x-1/2 mb-2 
                              bg-gray-900/90 text-white rounded-lg shadow-lg
                              min-w-[220px] p-3 z-50">
									{/* Title */}
									<div className="font-semibold mb-2 text-sm">{poi.title}</div>

									{/* Type */}
									<div className="text-xs text-gray-300 mb-2">{poi.type.name}</div>
