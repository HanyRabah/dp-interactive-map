...
import { POIColors, POILabels, POIMarkers } from "./markers";
...
const POIFilterPanel: React.FC<POIFilterPanelProps> = ({ pois, onPOIClick, project }) => {
	const [showFilter, setShowFilter] = useState(false);
	const showFilterDropDown = false;

	const poiTypes = useMemo(() => {
		return Array.from(new Set(pois.map(poi => poi.type.name)));
	}, [pois]);

	const [activeFilters, setActiveFilters] = useState(Object.fromEntries(poiTypes.map(type => [type, true])));
	...
	return (
		<>
			...
			<GlobePOIMarkers pois={filteredPois} onPOIClick={onPOIClick} showMarkers={true} project={project} />
		</>
	);
};
...
type FilterDropDownPorps = {
	showFilter: boolean;
	setShowFilter: (showFilter: boolean) => void;
	activeFilters: Record<string, boolean>;
	toggleFilter: (type: keyof typeof POILabels) => void;
};

const FilterDropDown = ({ showFilter, setShowFilter, activeFilters, toggleFilter }: FilterDropDownPorps) => {
	return (
		<div className="fixed top-24 left-4 w-[180px]">
					...
					<div className="bg-black/40 rounded-b-lg shadow-lg p-2 ">
						{Object.entries(activeFilters).map(([type]) => {
							...
							return (
								<div
									key={type}
									className={`group flex p-1 mb-1 rounded-lg items-center space-x-2 cursor-pointer hover:bg-white ${
										activeFilters[type as keyof typeof POILabels] && "bg-gray-800"
									}`}
